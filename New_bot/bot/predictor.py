...
from static_text import NON_LABELS_TEXT

#Register custom COCO dataset
register_coco_instances("clothes", {}, "./dataset/_annotations.coco.json", "./dataset")
...
cfg.MODEL.WEIGHTS = ("./model/model_777.pth")
    ...
    for i in range(len(outputs["instances"].pred_boxes)):
        ...
        for j in range(len(outputs["instances"].pred_boxes)):
            ...
            if i != j:
                ...
                if iou > 0.3:
                    if (outputs["instances"].scores[i] < outputs["instances"].scores[j]):
...
def predict(photo_name):
    ...
    bio.name = 'image.jpeg'
    bio.seek(0)
    text = sorted(set(np.array(outputs["instances"][instances].pred_classes)), reverse=True)
    if text == []:
        text = NON_LABELS_TEXT
    return bio, text
